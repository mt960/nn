...
UCSC - ASL
Modified: Added comprehensive lane change statistics tracking
"""
...
class CarlaGymEnv(gym.Env):
    ...
    def __init__(self, reward_system_type='baseline'):
        self.__version__ = "9.9.2"

        # Reward system type
        self.reward_system_type = reward_system_type
        ...
        self.lane_change_stats = {
            ...
            'distances': [],
            'reasons': {
                'avoid_slow_vehicle': 0,
                'overtake': 0,
                'exploration': 0,
                'unknown': 0
            }
        }
        self.current_lane_change = {
            'active': False,
            'start_time': 0,
            'start_s': 0,
            'start_d': 0,
    ...
    def _determine_lane_change_reason(self, vehicle_ahead, ego_speed):
        """Determine the reason for lane change"""
        if vehicle_ahead is not None:
            vehicle_ahead_speed = get_speed(vehicle_ahead)
            if vehicle_ahead_speed < ego_speed * 0.8:  # 前车速度显著较慢
                return 'avoid_slow_vehicle', vehicle_ahead_speed
            elif vehicle_ahead_speed < ego_speed:
                return 'overtake', vehicle_ahead_speed
        return 'exploration', None

    def _start_lane_change_tracking(self, ego_s, ego_d, target_d, vehicle_ahead):
        """Start tracking a new lane change"""
        if not self.current_lane_change['active']:
            ego_speed = get_speed(self.ego)
            reason, vehicle_ahead_speed = self._determine_lane_change_reason(vehicle_ahead, ego_speed)
            ...
            self.lane_change_stats['reasons'][reason] += 1

    def _finish_lane_change_tracking(self, ego_s, ego_d, success=True, abandoned=False):
        """Finish tracking current lane change"""
        if self.current_lane_change['active']:
            duration = time.time() - self.current_lane_change['start_time']
            distance = abs(ego_s - self.current_lane_change['start_s'])

            self.lane_change_stats['durations'].append(duration)
            self.lane_change_stats['distances'].append(distance)

            if abandoned:
                self.lane_change_stats['abandoned'] += 1
            elif success:
                self.lane_change_stats['successful'] += 1
            else:
                self.lane_change_stats['failed'] += 1

            # Reset current lane change
            self.current_lane_change = {
                'active': False,
                'start_time': 0,
                'start_s': 0,
    ...
    def get_lane_change_stats(self):
        """Return lane change statistics summary"""
        avg_duration = np.mean(self.lane_change_stats['durations']) if self.lane_change_stats['durations'] else 0
        avg_distance = np.mean(self.lane_change_stats['distances']) if self.lane_change_stats['distances'] else 0

        total = self.lane_change_stats['total_attempts']
        successful = self.lane_change_stats['successful']
        ...
        return {
            ...
            'failed': self.lane_change_stats['failed'],
            'abandoned': self.lane_change_stats['abandoned'],
            'avg_duration_seconds': round(avg_duration, 2),
            'avg_distance_meters': round(avg_distance, 2),
            'reasons': self.lane_change_stats['reasons'].copy()
        }

    def print_lane_change_summary(self):
        """Print formatted lane change statistics"""
        stats = self.get_lane_change_stats()

        print('\n' + '=' * 80)
        print('LANE CHANGE STATISTICS SUMMARY')
        print('=' * 80)
        print(f"Total Attempts:      {stats['total_attempts']}")
        print(f"Successful:          {stats['successful']} ({stats['success_rate_percent']}%)")
        print(f"Failed:              {stats['failed']}")
        print(f"Abandoned:           {stats['abandoned']}")
        print(f"Avg Duration:        {stats['avg_duration_seconds']}s")
        print(f"Avg Distance:        {stats['avg_distance_meters']}m")
        print('\nReasons:')
        print(f"  avoid_slow_vehicle : {stats['reasons']['avoid_slow_vehicle']}")
        print(f"  overtake           : {stats['reasons']['overtake']}")
        print(f"  exploration        : {stats['reasons']['exploration']}")
        print(f"  unknown            : {stats['reasons']['unknown']}")
        print('=' * 80 + '\n')
    ...
    def step(self, action=None):
        ...
        if self.lanechange and not self.current_lane_change['active']:
            initial_s = self.motionPlanner.estimate_frenet_state(ego_state, self.f_idx)[0]
            initial_vehicle_ahead = self.get_vehicle_ahead(initial_s, ego_init_d, ego_init_d, ego_target_d)
            self._start_lane_change_tracking(initial_s, ego_init_d, ego_target_d, initial_vehicle_ahead)
        ...
        while self.f_idx < wps_to_go and (elapsed_time(path_start_time) < self.motionPlanner.D_T * 1.5 or
            ...
            if self.current_lane_change['active']:
                d_diff = abs(ego_d - self.current_lane_change['target_d'])
                if d_diff < 0.5:  # Successfully reached target lane
                    self._finish_lane_change_tracking(ego_s, ego_d, success=True)

            # if ego off-the road or collided
            if any(collision_hist):
                collision = True
                # Mark lane change as failed if active
                if self.current_lane_change['active']:
                    self._finish_lane_change_tracking(ego_s, ego_d, success=False)
                break
        ...
        if collision:
            ...
            reward = self.off_the_road_penalty
            # done = True
            # Mark lane change as abandoned if active
            if self.current_lane_change['active']:
                self._finish_lane_change_tracking(self.actor_enumerated_dict['EGO']['S'][-1],
                                                 self.actor_enumerated_dict['EGO']['D'][-1],
    ...
    def destroy(self):
        ...
        # Print lane change statistics before destroying
        self.print_lane_change_summary()
